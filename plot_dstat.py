...
def plot_cpu_usage(data, output_image):
    ...
    # Plot usr CPU time
    plt.plot(time, usr, marker='o', linestyle='-', color='r', label='User CPU Util (%)')
    plt.plot(time, sys, marker='x', linestyle='--', color='b', label='Sys CPU Util (%)')
...
def plot_memory_usage(data, output_image):
    ...
    plt.plot(time, used_percentage, marker='x', linestyle='--', color='r', label='Memory Used (%)')

    plt.xlabel('Time(seconds)')
    plt.ylabel('Memory Used (%)')
    plt.title('Memory Used Percentage Over Time')
    plt.legend()
    plt.grid(True)
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python plot_dstat.py <dstat_file> <cpu_image> <memory_image>")
        sys.exit(1)

    dstat_file = sys.argv[1]
    cpu_image = sys.argv[2]
    ...
    plot_cpu_usage(data, cpu_image)
    plot_memory_usage(data, memory_image)
